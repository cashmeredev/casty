...
function buildStealthScript(lang, { fakeMedia = false, mediaPort = 0 } = {}) {
...
` : fakeMedia ? `
// getUserMedia emulation with fake streams (black video + silent audio)
if (navigator.mediaDevices) {
  navigator.mediaDevices.getUserMedia = (constraints) => {
    const tracks = [];
    if (constraints.video) {
...
export async function setupPage({ port, wsUrl }, { width, height, zoom = 1, mediaPort = 0 } = {}) {
  ...
  await Promise.all([
    ...
    client.send('Page.addScriptToEvaluateOnNewDocument', { source: buildStealthScript(lang, { fakeMedia: config.fakeMedia, mediaPort }) }),
