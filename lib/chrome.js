...
export function findChrome() {
  // Explicit override: any Chromium-based browser that accepts --headless=new
  // and --remote-debugging-port works (e.g. an already-installed Chromium,
  // Helium, Brave, …).  Treated as a full browser, so --headless=new is added.
  const override = process.env.CASTY_CHROME;
  if (override && existsSync(override)) return { bin: override, headless: true };
