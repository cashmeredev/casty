// Vimium 風ヒントモード
// Alt+F でクリック/フォーカス可能な要素にラベルを表示し、キー入力で選択
...
// 要素収集 + ラベル生成 + オーバーレイ注入を1回の evaluate でまとめて実行
...
export class HintMode {
  constructor(forceCapture) {
    ...
    this._client = null;
    this._hints = [];
    this._forceCapture = forceCapture || (() => {});
  }

  // ヒントモード開始 → ユーザーが選択 or キャンセルするまで待つ
  async start(client) {
    this._client = client;
    this.buffer = '';

    // 1回の evaluate で要素収集 + ラベル生成 + オーバーレイ注入
    let hints;
    try {
