...
export function startInputHandling(client, cellWidth, cellHeight, bindings, pauseRender, forceCapture) {
  ...
  // macOS Option キー → rawBindings を構築 (macOS のみ)
  // Linux では ESC+文字 が Alt+文字 と同じバイト列になるため衝突する
  const rawBindings = {};
  ...
  // ESC プレフィックスバッファ: ESC が単独で届いた場合、後続文字を待つ
  // Linux ターミナルでは Alt+Key が ESC + Key の2バイトで届くが、
  // async ハンドラの遅延で分割されることがある
  let escBuf = '';
  let escTimer = null;
