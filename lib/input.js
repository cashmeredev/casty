...
export async function dispatchMouse(client, type, x, y, button = 'left', clickCount = 0) {
  await client.send('Input.dispatchMouseEvent', { type, x, y, button, clickCount });
}

// Send scroll event via CDP
export async function dispatchScroll(client, x, y, deltaX, deltaY) {
...
export async function dispatchKey(client, { key, code, keyCode, text, modifiers = 0 }) {
...
export async function getHistoryEntry(client, delta) {
...
// ── Embed-mode action layer (driven by IPC, not stdin) ──

// Key name → CDP key info, for the IPC `key` command.  Printable single
// characters not listed here are synthesized on the fly in `key()` below.
const KEY_BY_NAME = {
  ...
  Backspace:  { key: 'Backspace',  code: 'Backspace',  keyCode: 8,  text: '\x08' },
  Delete:     { key: 'Delete',     code: 'Delete',     keyCode: 46 },
  Escape:     { key: 'Escape',     code: 'Escape',     keyCode: 27 },
  Space:      { key: ' ',          code: 'Space',      keyCode: 32, text: ' ' },
};

// ["ctrl","alt","shift","meta"] → CDP modifier bitmask
function modBitsFromNames(names) {
  const s = new Set((names || []).map(n => n.toLowerCase()));
  return modifierBits({ ctrl: s.has('ctrl'), alt: s.has('alt'), shift: s.has('shift'), meta: s.has('meta') });
}

// Build a reusable action surface over a CDP client, callable from the IPC
// server in embed mode.  Shares the low-level dispatch helpers with the
// interactive stdin path; the high-level URL-bar/hint UI of
// `startInputHandling` is intentionally NOT involved (embed mode draws its own
// chrome on the host side).
//
//   topRow : terminal row of the first content cell.  Interactive mode uses 2
//            (URL bar occupies row 1); embed mode uses 1 (no URL bar), so
//            mouse coordinates must not subtract the bar.
// _cw/_ch are CSS pixels per terminal cell (cssWidth/cols, cssHeight/rows).
// CDP Input.dispatchMouseEvent coordinates are in CSS pixels relative to the
// viewport, NOT device pixels -- so cell N maps to CSS x = (N-0.5)*cssCellW.
export function createActions(client, { forceCapture = () => {}, cellWidth = 8, cellHeight = 16, hintMode = null, topRow = 1 } = {}) {
  let _cw = cellWidth;
  let _ch = cellHeight;

  // Cell top-left → CSS pixel, honouring the content top row.
  const toPixel = (col, row) => ({ x: (col - 1) * _cw, y: (row - topRow) * _ch });
  // Centre of a cell, for precise clicks on links/buttons.
  const cellCentre = (col, row) => ({ x: (col - 0.5) * _cw, y: (row - topRow + 0.5) * _ch });
  // Default scroll/anchor point: a little inside the viewport so the wheel
  // event lands on the scrollable document rather than the corner.
  const anchor = () => ({ x: 2 * _cw, y: 2 * _ch });

  return {
    setCellSize(w, h) { if (w) _cw = w; if (h) _ch = h; },
    ...
    async scroll(dx = 0, dy = 0, col, row) {
      const p = (col != null && row != null) ? toPixel(col, row) : anchor();
      await dispatchScroll(client, p.x, p.y, dx, dy);
      forceCapture();
    },
    async key(name, modifiers) {
      const mods = modBitsFromNames(modifiers);
      let info = KEY_BY_NAME[name];
      if (!info && typeof name === 'string' && name.length === 1) {
        // Single printable char.  Send text only when no ctrl/alt is held.
        const upper = name.toUpperCase();
        info = {
          key: name,
          code: `Key${upper}`,
          keyCode: upper.charCodeAt(0),
          text: (mods & 0b0011) ? undefined : name, // bits: alt=1, ctrl=2
        };
      }
      if (!info) return;
      await dispatchKey(client, { ...info, modifiers: mods });
      forceCapture();
    },
    async text(string) {
      await client.send('Input.insertText', { text: string });
      forceCapture();
    },
    async click(col, row, button = 'left') {
      const { x, y } = cellCentre(col, row);
      await dispatchMouse(client, 'mousePressed', x, y, button, 1);
      await dispatchMouse(client, 'mouseReleased', x, y, button, 1);
      forceCapture();
    },
    async mouse(type, col, row, button = 'left') {
      const { x, y } = toPixel(col, row);
      await dispatchMouse(client, type, x, y, button);
    },
    async hints() {
      if (hintMode) hintMode.start(client); // resolves on selection/cancel
    },
    async hintKey(key) {
      if (hintMode) await hintMode.handleInput(key);
      return hintMode ? hintMode.active : false;
    },
    async getUrl() {
      const { currentIndex, entries } = await client.send('Page.getNavigationHistory');
      return entries[currentIndex] ? entries[currentIndex].url : '';
    },
  };
}
