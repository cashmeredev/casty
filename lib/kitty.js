...
// Placement: terminal cell the frame is anchored to, and the kitty image id(s).
...
// window) with id(s) allocated by the host to avoid collisions.
let _top = 2;
let _left = 1;

// Double buffering: when the host provides TWO image ids, each frame is
// transmitted+displayed under the next id and the previous id is deleted
// AFTER the new one is on screen, so the image is never momentarily cleared
// (no flicker on update).  With a single id we keep the original in-place
// replacement (interactive full-screen mode).
let _ids = [1];
let _cur = 0;
let _double = false;
...
// idB enables flicker-free double buffering when distinct from idA.
export function setPlacement(top, left, idA, idB) {
  if (top != null) _top = top;
  if (left != null) _left = left;
  if (idA != null) _ids[0] = idA;
  if (idB != null && idB !== _ids[0]) { _ids[1] = idB; _double = true; }
}
...
// Current target image id for this frame.
function curId() {
  return _ids[_cur];
}
...
// After displaying under the current id, return the escape that removes the
// previous buffer's image, and advance to the other buffer.  Empty when not
// double buffering.
function swapSeq() {
  if (!_double) return '';
  const old = _ids[_cur ^ 1];
  _cur ^= 1;
  return wrapKitty(deleteSeq(old));
}
...
function sendFrameFile(base64Data) {
  ...
  const seq = `\x1b_Ga=T,f=100,t=f,q=2,C=1,i=${curId()}${crFile};${tmpPathB64}\x1b\\`;
  process.stdout.write(`${placement()}${wrapKitty(seq)}${swapSeq()}`);
}
...
function sendFrameInline(pngBase64) {
  ...
  if (pngBase64.length <= CHUNK) {
    const seq = `\x1b_Ga=T,f=100,q=2,C=1,i=${curId()}${crInline};${pngBase64}\x1b\\`;
    process.stdout.write(`${placement()}${wrapKitty(seq)}${swapSeq()}`);
    return;
  }
  const parts = [];
  ...
  while (i < pngBase64.length) {
    ...
    if (i === 0) {
      parts.push(`\x1b_Ga=T,f=100,q=2,C=1,i=${curId()}${crInline},m=${more};${chunk}\x1b\\`);
    } else {
      parts.push(`\x1b_Gm=${more};${chunk}\x1b\\`);
    }
    i += CHUNK;
  }
  process.stdout.write(`${placement()}${wrapKitty(parts.join(''))}${swapSeq()}`);
}
